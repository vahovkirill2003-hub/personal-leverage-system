...
def seed_case(connection: psycopg.Connection, *, state: str = "EXECUTION") -> UUID:
    ...
    Version 0 is the state of a case *before* genesis: the creation transition
    of `14 v0.3` §3.3 is the only write with `case_version_before = 0`, and it
    carries `from_state IS NULL`.  A case row that already exists therefore
    starts at version 1, and an ordinary transition out of it advances to 2.
    """
    case_id = uuid7()
