...
## Data Model and Persistence Specification v0.1

**Статус:** кандидат на пользовательскую приёмку (проверка по упрощённому контуру `15` выполнена, находки DMV-1–DMV-3 закрыты)  
**Дата:** 6 августа 2026  
**Основание:** архитектурный baseline `11-technical-architecture-v1.1.md` (168448 байт, `faf6e5…30c22`), акт `13-architecture-baseline-acceptance-v0.md` (8073 байта, `0d9454…79aa`), продуктовый baseline v2 и решения `PLS-001–PLS-066`  
...
|---|---|---|---|
| `case` | id; created_at; current_state; current_status (null до терминала); case_version; current_revision_id→dossier_revision; depth_level (min/std/elevated); timezone; terminal_locked_at; diagnostic_hold (bool, reason_event_id) | `case_version` строго возрастает; CHECK: current_status задан ⇔ current_state терминально; при terminal_locked_at любые UPDATE кроме diagnostic-полей отклоняются rule-ом | P |
...
| `pre_experiment_dismissal` | id; original_request_ref; dismissed_at; stage (INTAKE/CLARIFICATION/FACT_UNKNOWN_MAPPING); reason (закрытый enum пяти оснований `PLS-063`); applied_rule; actual_system_time_sec; user_confirmation_event_id (null, обязателен только при зависимости от выбора); no_external_action, no_expense, no_obligation (bool, все true); disposition | Не FK-обязана case-у в терминальном смысле: не создаёт статуса; CHECK все три «отсутствия» = true | A |
...
| `state_transition` | id; case_id; event_id; from_state; to_state; guard_results_ref (правило, версия, факты, вердикт по каждому guard); case_version_before/after | Пара (case_id, case_version_after) UNIQUE; запись только в транзакции, обновляющей `case` | A |
...
| `evidence_retention_policy` | case_id; состав; форма; цель; trigger (срок/событие) | Обязательна до первого UA (`PLS-054`): guard `G…` первого UA проверяет наличие | P до UA, затем I |
...
| Вход | Ключ | Гарантия |
|---|---|---|
| Telegram update | (bot_id, update_id) | Один processing result |
...
|---|---|---|
| `pls_web` | web | INSERT `inbox`; SELECT projections/read models; EXECUTE только application commands постановки (enqueue): без прямых INSERT в gate_record/consent/authority_grant/experiment_anchor |
| `pls_worker` | worker | Полный набор application commands через engine-модули: INSERT доменных A-таблиц, UPDATE projections; без DDL; без DELETE (кроме retention-процедуры отдельной ролью) |
