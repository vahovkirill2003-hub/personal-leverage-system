...
| `ADDITIONAL_TIME_APPROVED` | +1 | используется таблицей `03` §10.1, но отсутствует в каталоге §4 — см. ниже |
| Технические триггеры прозаических правил | +17 | перечислены ниже |
| **Итого закрытый тип** | **66** | каждый триггер связан минимум с одним правилом каталога |
...
**События каталога `03` §4, не связанные ни с одним правилом** (5): `EXTERNAL_ACTION_REQUESTED`, `PACKAGE_BOUNDARY_REACHED`, `NON_MATERIAL_CHANGE_RECORDED`, `RELEVANCE_CONFIRMED`, `LATE_EVIDENCE_RECORDED`. Они записываются в ledger как журнальные события и правил не запускают; в закрытый тип входных триггеров `execute_trigger` не входят.

**`ADDITIONAL_TIME_APPROVED`.** Событие используется нормативной таблицей `03` §10.1 (правило R67), но в каталоге событий `03` §4 отсутствует. Это ранее зафиксированная незакрытая находка `AUD-N2` («несвязанные и отсутствующие события каталога», `06` §7, MINOR). Настоящая спецификация регистрирует его как триггер, поскольку без него правило R67 неисполнимо, и **не** добавляет его в продуктовый каталог: пополнение `03` §4 — предмет отдельной продуктовой errata к следующей версии `03`, а не решения стадии реализации. До неё расхождение остаётся зафиксированным здесь и в отчётности как известное.
...
### 3.3. Fail-closed свойства протокола
...
| Добавлен §1.4 TriggerRegistry: закрытый тип из 66 входных триггеров, технические идентификаторы 17 прозаических правил, статус `ADDITIONAL_TIME_APPROVED` как незакрытой `AUD-N2`, две пары коллизий и их дискриминаторы | `PLS-069` |
